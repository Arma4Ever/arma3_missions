...
private _unitName = _unit call a3cs_common_fnc_getUnitName;

if (isPlayer _unit && {!(_player isEqualTo _unit)}) then {
    private _playerName = _player call a3cs_common_fnc_getUnitName;
    private _hintText = format [LLSTRING(Action_CheckGear_Notification), _playerName];
    _hintText remoteExecCall ["hint", _unit];
};

//set header title
(_display displayCtrl IDC_LISTBOX_HEADER) ctrlSetText format [LLSTRING(Action_CheckGear_ListHeader), _unitName];
//setup close button
(_display displayCtrl IDC_LISTBOX_BUTTON3) ctrlSetText "Zamknij";
