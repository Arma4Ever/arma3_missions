...
fn_crashNow = {
	if (isServer) then{
		...
		0=[_heli,_survive] spawn fn_fnc_heliInit;
		};
};
...
_crewMembers=(_this select 1);
	...
	for "_h" from 0.4 to 1.1 step 0.1 do {
	 ...
	 if (_hitPoint == "HitEngine")then{
			[[_heli,"engineBang"],"fn_fnc_say2D",_crewMembers] call BIS_fnc_MP;
...
[[_heli,_sound],"fn_fnc_say2D",_crewMembers] call BIS_fnc_MP;
[[_power,_freq],"fn_fnc_camShake",_crewMembers] call BIS_fnc_MP;
...
private ["_heli"];
    ...
    if (local _heli) then {
		...
		if (_survive) then {
	[[],"fn_fnc_handleDamage",_crewMembers] call BIS_fnc_MP;
					};
					
	0=[_heli,_crewMembers] spawn fn_fnc_setHit;
...
waitUntil {(((getPosATL _heli) select 2) < 15) OR (velocity _heli select 2 < - 13) OR ((getPosASLW _heli)select 2) < 10};
	...
	[[_heli,"pullUp"],"fn_fnc_say2D",_crewMembers] call BIS_fnc_MP;
...
waitUntil {(((getPosATL _heli) select 2) < 2) OR time > _startTime OR ((getPosASLW _heli) select 2) < 1}; 
	...
	_heli lock false;
		...
		if (_survive)then{
	[[_heli],"fn_fnc_eject",_crewMembers] call BIS_fnc_MP;
			};
			
	_heli setDamage 1;
	
		[[_heli],"fn_fnc_smoke",true] call BIS_fnc_MP;
...
"PVsetPilot" addPublicVariableEventHandler {
    (_this select 1) spawn fn_fnc_heliInit;
};
