...
#define mission_FONT			"PuristaBold"
#define mission_FONTSIZE		0.03
#define mission_OFFSET		0.011
#define mission_ELEMENT_X		0.0705
#define mission_ELEMENT_Y		0.0305
#define mission_ELEMENT_W		0.13
#define mission_ELEMENT_H		0.05

class GVAR(GUI_List): RscListBox {
  type = CT_LISTBOX;
  style = ST_MULTI;
  font = mission_FONT;
  sizeEx = mission_FONTSIZE;
  color[] = {1, 1, 1, 1};
...
class GVAR(GUI_TargetMenu) {
	...
	onLoad = "0 spawn mission_fnc_restoreDisplay";
	onUnload = "call mission_fnc_saveDisplay";
	...
	class controls
	{
		...
		class GVAR(TEXTTarget): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 5.475);
			y = mission_OFFSET + (mission_ELEMENT_Y * 7.5);
			w = (mission_ELEMENT_W * 1.5);
			h = mission_ELEMENT_H;
			Size = 0.1;
		...
		class GVAR(TEXTOwnGrid): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 2.74);
			y = mission_OFFSET + (mission_ELEMENT_Y * 10.3);
			w = (mission_ELEMENT_W * 1.2);
			h = mission_ELEMENT_H;
		...
		class GVAR(TEXTOwnGridX): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 2.8);
			y = mission_OFFSET + (mission_ELEMENT_Y * 12);
			w = (mission_ELEMENT_W * 0.5);
			h = mission_ELEMENT_H;
		...
		class GVAR(TEXTOwnGridY): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 2.8);
			y = mission_OFFSET + (mission_ELEMENT_Y * 14);
			w = (mission_ELEMENT_W * 0.5);
			h = mission_ELEMENT_H;
		...
		class GVAR(TBoxOwnGridX): GVAR(GUI_TextBox) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 3.4);
			y = mission_OFFSET + (mission_ELEMENT_Y * 12);
			w = (mission_ELEMENT_W * 0.6);
			h = mission_ELEMENT_H;
		...
		class GVAR(TEXTTargetGrid): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 5.74);
			y = mission_OFFSET + (mission_ELEMENT_Y * 10.3);
			w = (mission_ELEMENT_W * 1.2);
			h = mission_ELEMENT_H;
		...
		class GVAR(TEXTTargetGridX): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 5.8);
			y = mission_OFFSET + (mission_ELEMENT_Y * 12);
			w = (mission_ELEMENT_W * 0.5);
			h = mission_ELEMENT_H;
		...
		class GVAR(TBoxTargetGridX): GVAR(GUI_TextBox) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 6.4);
			y = mission_OFFSET + (mission_ELEMENT_Y * 12);
			w = (mission_ELEMENT_W * 0.6);
			h = mission_ELEMENT_H;
		...
		class GVAR(ButtonTargetDirMil): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 10);
			y = mission_OFFSET + (mission_ELEMENT_Y * 12);
			w = (mission_ELEMENT_W * 0.4);
			h = mission_ELEMENT_H;
		...
		class GVAR(ButtonTargetElev): GVAR(GUI_Text) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 10);
			y = mission_OFFSET + (mission_ELEMENT_Y * 16);
			w = (mission_ELEMENT_W * 0.4);
			h = mission_ELEMENT_H;
		...
		class GVAR(TBoxTargetPolarDir): GVAR(GUI_TextBox) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 10.8);
			y = mission_OFFSET + (mission_ELEMENT_Y * 12);
			w = (mission_ELEMENT_W * 0.6);
			h = mission_ELEMENT_H;
		...
		class GVAR(ButtonTargetApply): GVAR(GUI_Button2) {
			...
			x = mission_OFFSET + (mission_ELEMENT_X * 6);
			y = mission_OFFSET + (mission_ELEMENT_Y * 18);
			w = (mission_ELEMENT_W * 0.6);
			
			text = "Wylicz";
			tooltip = "Oblicza dystans oraz kierunek do celu, a także elewację oraz ładunek";
			onButtonClick = "playSound 'click'; 0 call mission_fnc_calculate";
		};

		class GVAR(Charge): GVAR(GUI_List) {
			idc = 47519;
			
			x = mission_OFFSET + (mission_ELEMENT_X * 1);
			y = mission_OFFSET + (mission_ELEMENT_Y * 5.2);
			w = (mission_ELEMENT_W * 1);
			h = 0.1;
