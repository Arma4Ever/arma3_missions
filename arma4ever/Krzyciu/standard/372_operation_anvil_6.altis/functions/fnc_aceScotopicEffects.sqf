...
if ((ace_common_OldIsCamera) || {!alive _unit} || {currentVisionMode _unit != 0}) exitWith {
    GVAR(scotopicCC) ppEffectEnable false;
    [FUNC(aceScotopicEffects), [0], 0.1] call CBA_fnc_waitAndExecute;
};
...
if ((_light > 50) && {(param [0, 0]) == 0}) exitWith { // make sure we've faded out before disabling
    ...
    [FUNC(aceScotopicEffects), [0], [TIME_INTERVAL, 60] select (_ambientLight > 100)] call CBA_fnc_waitAndExecute;
};
...
[FUNC(aceScotopicEffects), [_intensity], TIME_INTERVAL] call CBA_fnc_waitAndExecute;